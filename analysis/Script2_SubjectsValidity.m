...
summary_allfusion=zeros(max(data.SubjectInd),max(data.trialtype)+1);
for sub_ind=1:max(data.SubjectInd)
    summary_allfusion(sub_ind,1)=mean(data.SubjectCode(data.SubjectInd==sub_ind));
    for trialtype=1:max(data.trialtype)
        summary_allfusion(sub_ind,trialtype+1)=mean(data.AllFusion(data.SubjectInd==sub_ind & data.trialtype==trialtype));
    end
end
...
allfusion_sd = std(summary_allfusion_valid_participants);

fprintf('\nCorrupted trials: M = %.2f%%, SD = %.2f%%\n',iscorrupted_mean*100, iscorrupted_sd*100)
fprintf('All Fusion trials: M = %.2f%%, SD = %.2f%%\n',allfusion_mean*100, allfusion_sd*100)
