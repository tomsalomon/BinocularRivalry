...
firstseen=BR_data.Initial_face;
firstseen(BR_data.face1Time+BR_data.face2Time==0)=nan;
N=length(unique(BR_data.Subject));
% time_diff_mat=mean(reshape(time_diff,[length(time_diff)/N],N));
time_diff_mat=reshape(firstseen,[length(firstseen)/N,N]);
time_diff_mean=nanmean(time_diff_mat)-0.5;

direction=sign(mean(firstseen));
visualize=false;
onetailed=false;
n=0;
...
while stop_cond>0
    ...
    for itr=1:power_iterations
        ...
        rand_sample_data=time_diff_mat(:,rand_sample);
